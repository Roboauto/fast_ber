...
TEST_CASE("Identifier: Encode ExplicitIdentifier")
{
    ...
    REQUIRE(size == 3);
    REQUIRE(enc_size == 3);
    REQUIRE(std::span(buffer.data(), 3) == std::span(expected));
}
...
TEST_CASE("Identifier: Encode TaggedExplicitIdentifier")
{
    ...
    REQUIRE(size == 5);
    REQUIRE(enc_size == 5);
    REQUIRE(std::span(buffer.data(), 5) == std::span(expected));
}
...
TEST_CASE("Identifier: Decode ExplicitIdentifier")
{
    ...
    auto                   iterator = fast_ber::BerViewIterator(std::span<uint8_t>(data.data(), data.size()));

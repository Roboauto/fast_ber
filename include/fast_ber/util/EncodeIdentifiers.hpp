...
inline size_t           encode_tag(std::span<uint8_t> output, Tag tag) noexcept;
inline size_t           encode_tag(std::span<uint8_t> output, UniversalTag tag) noexcept;
...
// Return the size of the encoded tag field or zero on fail
inline size_t encode_identifier(std::span<uint8_t> output, Construction construction, Class class_, Tag tag) noexcept;
inline size_t encode_identifier(std::span<uint8_t> output, Construction construction, Class class_,
...
// Return the size of the encoded tag field or zero on fail
inline size_t           encode_length(std::span<uint8_t> output, uint64_t length) noexcept;
...
inline size_t encode_header(std::span<uint8_t> output, Construction construction, Class class_, Tag tag,
                            size_t length) noexcept;
inline size_t encode_header(std::span<uint8_t> output, Construction construction, Class class_, UniversalTag tag,
                            size_t length) noexcept;
...
inline size_t encode_length(std::span<uint8_t> output, uint64_t length) noexcept
{
    if (length < 0x80 && output.size() > 0)
    {
        output[0] = (length >> 0) & 0xFF;
        return 1;
    }
    else if (length <= 0xFF && output.size() > 1)
    {
        output[0] = 0x81;
        output[1] = (length >> 0) & 0xFF;
        return 2;
    }
    else if (length <= 0xFFFF && output.size() > 2)
    {
        output[0] = 0x82;
    ...
    else if (length <= 0xFFFFFF && output.size() > 3)
    {
        output[0] = 0x83;
    ...
    else if (length <= 0xFFFFFFFF && output.size() > 4)
    {
        output[0] = 0x84;
    ...
    else if (length <= 0xFFFFFFFFFF && output.size() > 5)
    {
        output[0] = 0x85;
    ...
    else if (length <= 0xFFFFFFFFFFFF && output.size() > 6)
    {
        output[0] = 0x86;
    ...
    else if (length <= 0xFFFFFFFFFFFFFF && output.size() > 7)
    {
        output[0] = 0x87;
                            ...
                            size_t length) noexcept
{
    ...
    size_t length_length = encode_length(output.subspan(id_length), length);
